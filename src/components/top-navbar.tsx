                ...
                <div className="hidden w-full md:flex md:w-auto">
                    <ul className="flex flex-col font-medium p-4 md:p-0 mt-4 border  rounded-lg  md:space-x-8 rtl:space-x-reverse md:flex-row md:mt-0 md:border-0 bg-[#161615] md:bg-[#161615] border-gray-700">
                        ...
                        <li>
                            ...
                            <div className='hover:text-red-500 hover: flex gap-2'>
                                <button onClick={() => { mainComponent?.setMainComponent(MainComponentContextType.ABOUTUS) }} className=" hover:text-[#f9d112] py-2 px-3 text-white rounded md:bg-transparent md:p-0 font-semibold flex justify-center items-center">About Us
                                    <svg onClick={() => setShowServiceDropdown(!showServiceDropdown)} onBlurCapture={() => setShowServiceDropdown(false)} className="w-2.5 h-2.5 ms-2.5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 10 6">
                                        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 4 4 4-4" />
                                    </svg>
                                </button>
                                {showServiceDropdown && <div id="dropdownNavbar" className="z-10 absolute top-20 font-normal divide-y divide-gray-100 rounded-lg shadow w-44 bg-gray-700">
                                    <ul className="py-2 text-sm text-white" aria-labelledby="dropdownLargeButton">
                                        <li>
                                            <a onClick={() => {setShowServiceDropdown(!showServiceDropdown); mainComponent?.setMainComponent(MainComponentContextType.ABOUTUS) }} className="block px-4 py-2 hover:bg-gray-600 hover:text-[#f9d112]">Schedules</a>
                                        </li>
                                        <li>
                                            <a onClick={() => {setShowServiceDropdown(!showServiceDropdown); mainComponent?.setMainComponent(MainComponentContextType.ABOUTUS) }}  className="block px-4 py-2  hover:bg-gray-600 hover:text-[#f9d112]">Meet Team</a>
                                        </li>
                                    </ul>
                                </div>}
                            </div>
